...
function mvninit() {
  if [ $# != 3 ]; then
      ...
      TMPL_DIR=${TRIBF_SHELL_HOME}/dev.sh.d/templ/mvn
      ...
      mkdir -p $APP_ARTIFACT_ID/conf

      envsubst '${APP_GROUP_ID} ${APP_ARTIFACT_ID} ${JAVA_VERSION}' < ${TMPL_DIR}/pom.xml.$3.tmpl > $APP_ARTIFACT_ID/pom.xml
      envsubst '${APP_ARTIFACT_ID}' < ${TMPL_DIR}/app.xml.tmpl > $APP_ARTIFACT_ID/conf/app.xml
      ...
      cd $APP_ARTIFACT_ID && mvn compile
  fi
}
